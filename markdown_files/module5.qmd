...
```
  ...
  - `enterobase_assemblystats[enterobase_assemblystats$N50 < 4e+06,]` : This is how you **filter** a dataframe in R.
    ...
    - We are not filtering by columns, we want all columns so we leave it blank. That's why there is nothing after the **comma**
      - If you would like to filter by columns : `enterobase_assemblystats[enterobase_assemblystats$N50 < 4e+06,c(1,4,7)]` will print only the first, fourth and seventh column of the dataframe. After the **comma** we specify the columns we want, enclosed in `c()`. You can run `colnames(dataframe)` to see the names of all the columns (and the column number). Run `colnames(enterobase_assemblystats)` on the console and find the number for the N50 column!
...
Most the visual customizations for your plot can be supplied within the `theme` layer. Take a look at the code below and the resulting plot. Can you figure out what each line is doing? Try changing different values to see how it changes the plot!
...
ggplot(data = enterobase_assemblystats[enterobase_assemblystats$N50<4000000,],aes(x=`Contig Number(>=200 bp)`,y=N50))+
  ...
  labs(x="Number of contigs > 200bp",y="N50 value",title="Relationship between N50 value and number of contigs",subtitle="Data obtained from Enterobase Salmonella Minnesota assembly stats")
```

#### Faceting and multi-panel figures

##### Mutli-panel figures

Lets save two figures we have already made into ggplot objects

```{r}

scatterplotfig<-ggplot(data = enterobase_assemblystats[enterobase_assemblystats$N50<4000000,],aes(x=`Contig Number(>=200 bp)`,y=N50))+
  geom_point()

histogramfig<-ggplot(data = enterobase_assemblystats,aes(x=`Contig Number(>=200 bp)`))+
  geom_histogram(binwidth = 5)

```

Now, instead of the images being displayed on the **Viewer** pane, they are now saved as objects in the **Environment** pane.

Lets use these objects to make a multi-panel figure using the `plot_grid` function from the `cowplot` package

```{r}
plot_grid(scatterplotfig,histogramfig)
```

Take a look at the help page for `plot_grid`. Try to recreate the plot below by playing with the different options! 

```{r fig.height=7,fig.width=4, echo=F, include=T}
plot_grid(scatterplotfig,histogramfig,nrow=2,labels=c("A","B"))
```
<details>
<summary>Click here to see the answer </summary>
```{.r}
...
Faceting is when you make multiple plots with the same x and y axes for the same data, but grouped by a specific factor. For eg: Lets remake our Number of contigs vs N50 scatterplot, but now separated by different Continents.
...
Since we saved the scatterplot to a ggplot object called `scatterplotfig`, we dont have to rewrite the base ggplot code again, as it is all contained within the object already. We can simply continue to refer to the object and add more "layers" (or `geoms`) to it with the `+` sign. 

`facet_wrap` splits the data by a specific group, and here we specified `Continent` using `facet_wrap(Continent~.)` . 

`scales = "free"` will automatically adjust the x and y axis scales for each facet.
